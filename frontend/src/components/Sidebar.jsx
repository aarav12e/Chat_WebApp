    ...
    <aside className="h-full w-full sm:w-20 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200">
      ...
      <div className="border-b border-base-300 w-full p-4">
        <div className="flex items-center gap-2">
          ...
          {/* Show "Contacts" label on mobile (full width) and desktop lg+ */}
          <span className="font-medium sm:hidden lg:block">Contacts</span>
        </div>

        {/* Online filter — visible on mobile and lg desktop, hidden on sm icon-only mode */}
      ...
      <div className="overflow-y-auto w-full py-3">
        {filteredUsers.map((user) => (
          ...
          >
            ...
            {/* Name & status: full width on mobile, hidden on sm icon-only, visible on lg */}
